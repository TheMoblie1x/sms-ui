...
class GetEntityList {
    ...
    suspend fun getAllGroupsByCompany(companyID:Int): List<Groups>? {
    ...
    suspend fun getSurveySetsByGroupID(groupID:Int): List<SurveySet>? {
    ...
    suspend fun getSurveySetBySurveySetID(surveySetID:Int): List<SurveySet>? {
        val allSurveysAPI = apiClient.create(SurveySetsAPI::class.java)
        lateinit var data:List<SurveySet>
        val launch = withContext(Dispatchers.IO) { // Use a coroutine scope tied to a lifecycle
